...
class IntervalSeqTest extends AnyFunSuite {
  ...
  test("test06_Extent") {
    ...
    val intervalSequence1 = IntervalSeq(intervals.iterator.to(Vector))
    assert(intervalSequence1.extent.contains(Interval.closed(Limit(5), Limit(25))))

    intervals += _o18
    val intervalSequence2 = IntervalSeq(intervals.iterator.to(Vector))
    assert(intervalSequence2.extent.contains(Interval.closed(Limitless[Int](), Limit(25))))

    intervals += all
    val intervalSequence3 = IntervalSeq(intervals.iterator.to(Vector))
