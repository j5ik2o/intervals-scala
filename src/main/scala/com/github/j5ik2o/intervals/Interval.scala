...
class Interval[T](
  ...
  /** この区間の<b>補</b>区間と与えた区間 `other` の共通部分を返す。
    ...
    *   補区間と対照区間の共通部分のリスト
    * @see
    *   <a href="http://en.wikipedia.org/wiki/Set_theoretic_complement">complement (wikipedia)</a>
    */
  ...
  /** 区間をグラフィカルに確認するためのデバッグ用メソッド。
    ...
    * @code [}で表示する。 上側限界が開区間である場合はキャラクタ{ @code )}、閉区間である場合はキャラクタ`]`で表示する。 区間内の要素はキャラクタ`-`で表示する。
    *
    * @return
    *   文字列
    */
