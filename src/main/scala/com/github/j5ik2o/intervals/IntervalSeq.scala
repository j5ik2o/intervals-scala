...
/** 区間同士の比較を行うための`Ordering`の実装(上側優先)
  ...
  */
class UpperLowerOrdering[T](private val inverseLower: Boolean, private val inverseUpper: Boolean)(implicit
    ev: T => Ordered[T]
...
/** 区間同士の比較を行うための`Ordering`の実装(下側優先)
  ...
  * @param inverseUpper 上限が逆順の場合は`false`
  */
class LowerUpperOrdering[T](private val inverseLower: Boolean, private val inverseUpper: Boolean)(implicit
    ev: T => Ordered[T]
) extends Ordering[Interval[T]] {
...
/** 区間列（複数の [[Interval]] の列）を表すクラス。
  ...
  */
class IntervalSeq[T](val intervals: Seq[Interval[T]], val ordering: Ordering[Interval[T]])(implicit
    ev: T => Ordered[T]
) {
  def :+(value: Interval[T]): IntervalSeq[T] = new IntervalSeq(intervals :+ value)
  ...
  def iterator: Iterator[Interval[T]] = this.intervals.sorted(ordering).iterator
...
object IntervalSeq {
  ...
  /** 抽出子メソッド。
    ...
    * @return 構成要素
    */
  def unapply[T](intervalSeq: IntervalSeq[T])(implicit
      ev: T => Ordered[T]
  ): Option[(Seq[Interval[T]], Ordering[Interval[T]])] =
